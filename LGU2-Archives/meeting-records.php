...
$current_folder_name = 'Meeting/Sessions Records';
...
// Fetch Subfolders
$folders = [];
$folder_sql = "SELECT * FROM legislative_folders WHERE type = 'Meeting'";
if ($current_folder_id) {
...
// Fetch Files
$files = [];
$file_sql = "SELECT id, title, type, month, year, author, created_at, last_accessed, version, parent_version_id FROM legislative_records WHERE type = 'Meeting' AND parent_version_id IS NULL AND created_at >= DATE_SUB(NOW(), INTERVAL 5 YEAR)";
if ($current_folder_id) {
...
<body class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-900 dark:to-slate-800 text-gray-900 dark:text-gray-100 transition-colors duration-200">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8 pb-6 border-b border-gray-200 dark:border-slate-700">
            ...
            <nav class="flex text-sm font-medium text-gray-500 dark:text-gray-400 mt-2">
                <a href="meeting-records.php" class="hover:text-red-600 dark:hover:text-red-400 transition-colors">Root</a>
        ...
        <?php if ($current_folder_id): ?>
        <div class="mb-4">
            <a href="meeting-records.php<?php echo $parent_folder_id ? '?folder_id=' . $parent_folder_id : ''; ?>" class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">
                ...
                <?php else: ?>
                    <?php foreach ($files as $record): ?>
                        <div data-id="<?php echo $record['id']; ?>" 
                            ...
                            <div class="flex-shrink-0">
                                <i class="bi bi-file-earmark-text text-indigo-600 dark:text-indigo-400 text-2xl"></i>
                            </div>

                            <div class="flex-1 min-w-0">
                                <div class="font-medium text-gray-800 dark:text-gray-200 truncate"><?php echo htmlspecialchars($record['title']); ?></div>
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">
                                    <span class="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300 rounded text-[11px]"><?php echo htmlspecialchars($record['type']); ?></span>
                                    <span class="mx-2">•</span>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-slate-700">
                ...
                <form id="uploadForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="fileName" name="fileName" required class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100" placeholder="e.g., Meeting_Minutes.pdf">
                    </div>
                    <div>
    ...
    <div id="sideViewer" class="fixed right-0 top-0 h-full w-96 bg-white dark:bg-slate-900 border-l border-gray-200 dark:border-slate-700 shadow-xl transform translate-x-full transition-transform duration-200 z-50">
        ...
        <div class="p-4 space-y-3">
            ...
            <div id="sv-preview" class="mt-3"></div>
        </div>
