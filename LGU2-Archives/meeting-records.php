...
// Fetch Files
$files = [];
$file_sql = "SELECT id, title, type, month, year, author, created_at, last_accessed, parent_version_id FROM legislative_records WHERE type = 'Meeting' AND parent_version_id IS NULL AND created_at >= DATE_SUB(NOW(), INTERVAL 5 YEAR)";
if ($current_folder_id) {
...
try {
    ...
    $file_res = false;
    error_log("SQL Error in meeting-records: " . $e->getMessage());
}
if ($file_res) {
