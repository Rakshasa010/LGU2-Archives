...
$current_folder_name = 'Ordinances & Resolutions';
...
// Fetch Subfolders
$folders = [];
$folder_sql = "SELECT * FROM legislative_folders WHERE type IN ('Ordinance', 'Resolution')";
if ($current_folder_id) {
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8 pb-6 border-b border-gray-200 dark:border-slate-700">
            ...
            <nav class="flex text-sm font-medium text-gray-500 dark:text-gray-400 mt-2">
                <a href="ordinances-resolution.php" class="hover:text-red-600 dark:hover:text-red-400 transition-colors">Root</a>
        ...
        <?php if ($current_folder_id): ?>
        <div class="mb-4">
            <a href="ordinances-resolution.php<?php echo $parent_folder_id ? '?folder_id=' . $parent_folder_id : ''; ?>" class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">
            ...
            <div id="filesList" class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-gray-50/50 dark:bg-slate-800/20">
                ...
                <?php else: ?>
                    <?php foreach ($files as $record): 
                        ...
                        $typePrefix = ($record['type'] === 'Ordinance') ? 'ORD' : 'RES';
                        ...
                        <div data-id="<?php echo $record['id']; ?>" 
                            ...
                            <div class="p-4 flex flex-col flex-1">
                                ...
                                <div class="flex flex-wrap gap-2 mb-3">
                                    <span class="px-2 py-1 rounded <?php echo $typeClass; ?> text-xs font-medium"><?php echo htmlspecialchars($record['type']); ?></span>
                                ...
                                <div class="mt-auto pt-3 border-t border-gray-100 dark:border-slate-700">
                                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2 font-mono text-xs font-semibold text-blue-700 dark:text-blue-300 text-center tracking-wider">
                                        <?php echo $typePrefix; ?>-<?php echo str_pad($record['id'], 6, '0', STR_PAD_LEFT); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
