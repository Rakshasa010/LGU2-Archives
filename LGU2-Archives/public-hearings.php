...
$current_folder_name = 'Public Hearings';
...
// Fetch Subfolders
$folders = [];
$folder_sql = "SELECT * FROM legislative_folders WHERE type = 'Public Hearing'";
if ($current_folder_id) {
...
// Fetch Files
$files = [];
$file_sql = "SELECT id, title, type, month, year, author, created_at, last_accessed, parent_version_id FROM legislative_records WHERE type = 'Public Hearing' AND parent_version_id IS NULL";
if ($current_folder_id) {
...
try {
    ...
    $file_res = false;
    error_log("SQL Error in public-hearings: " . $e->getMessage());
}
if ($file_res) {
...
<body class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-900 dark:to-slate-800 text-gray-900 dark:text-gray-100 transition-colors duration-200">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8 pb-6 border-b border-gray-200 dark:border-slate-700">
            ...
            <nav class="flex text-sm font-medium text-gray-500 dark:text-gray-400 mt-2">
                <a href="public-hearings.php" class="hover:text-red-600 dark:hover:text-red-400 transition-colors">Root</a>
        ...
        <?php if ($current_folder_id): ?>
        <div class="mb-4">
            <a href="public-hearings.php<?php echo $parent_folder_id ? '?folder_id=' . $parent_folder_id : ''; ?>" class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">
            ...
            <div id="filesList" class="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-gray-50/50 dark:bg-slate-800/20">
                ...
                <?php else: ?>
                    <?php foreach ($files as $record): 
                        ...
                        $iconClass = 'bi-file-earmark-text text-green-500';
                        ...
                        <div data-id="<?php echo $record['id']; ?>" 
                            ...
                            <div class="p-4 flex flex-col flex-1">
                                    ...
                                    <div class="relative flex-shrink-0">
                                        ...
                                        <div id="record-menu-<?php echo $record['id']; ?>" class="hidden absolute right-0 top-full mt-1 w-44 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-slate-700 z-50 py-1 overflow-hidden">
                                            <button onclick="openSideViewerServer(<?php echo $record['id']; ?>, '<?php echo addslashes(htmlspecialchars($record['title'])); ?>', '<?php echo addslashes(htmlspecialchars($record['type'])); ?>', '<?php echo addslashes(htmlspecialchars($record['month'])); ?>', '<?php echo addslashes(htmlspecialchars($record['year'])); ?>', '<?php echo addslashes(htmlspecialchars($record['author'])); ?>', '<?php echo addslashes(htmlspecialchars($record['created_at'])); ?>', '<?php echo addslashes(htmlspecialchars($record['last_accessed'] ?? '')); ?>'); document.getElementById('record-menu-<?php echo $record['id']; ?>').classList.add('hidden');" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 flex items-center gap-2 border-b border-gray-100 dark:border-slate-700">
                                                <i class="bi bi-eye text-green-500"></i> View
                                            </button>
                                ...
                                <div class="flex flex-wrap gap-2 mb-3">
                                    <span class="px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 text-xs font-medium rounded"><?php echo htmlspecialchars($record['type']); ?></span>
                                ...
                                <div class="mt-auto pt-3 border-t border-gray-100 dark:border-slate-700">
                                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2 font-mono text-xs font-semibold text-blue-700 dark:text-blue-300 text-center tracking-wider">
                                        PH-<?php echo str_pad($record['id'], 6, '0', STR_PAD_LEFT); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-slate-700">
                ...
                <form id="uploadForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="fileName" name="fileName" required class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100" placeholder="e.g., Public_Hearing_Minutes.pdf">
                    </div>
                    <div>
    ...
    <script>
        ...
        function addUploadedFileToList(fileData) {
            ...
            const typeClass = 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
            const typePrefix = 'PH';
